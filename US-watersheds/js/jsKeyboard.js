var jsKeyboard = {
    ...
    init: function(elem, keyboard) {
        ...
        jsKeyboard.addKeyDownEvent();
         ...
         $(':input').not('[type="reset"]').not('[type="submit"]').on('focus, click', function(e)
         {
            if (currInput==1)
				{
				jsKeyboard.currentElement=$('#join_email2');
				jsKeyboard.currentElementCursorPosition=document.getElementById('join_email2').value.length;
				}
			else
				{
				jsKeyboard.currentElement=$('#join_zip2');
				jsKeyboard.currentElementCursorPosition=document.getElementById('join_zip2').value.length;
				}
    ...
    write: function(m) {
		currTime=0;
		if (currInput==1 || (currInput==2 && document.getElementById('join_zip2').value.length<5 && !isNaN(String.fromCharCode(m))))
			{
